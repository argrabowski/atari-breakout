...
import s2_mlp as s2_mlp
import os

pretrained_policy_path = "../trained_policies/s2_mlp_policy_network.pt"
new_policy_path="../trained_policies/s2_mlp_policy_network.pt"

batch_size = 12                             # Number of experiences to sample from the replay buffer for each training iteration
...
episodes = 2000000                          # Total number of episodes to train for
memory_size = 500000                        # Maximum size of the replay buffer
...
num_eval_episode = 5                        # Number of episodes to evaluate the policy network on during evaluation
random_exploration_interval = 100000        # Number of steps during which to perform random exploration before using the policy network
...
def evaluate(step, policy_net, device, env, n_actions, train):
    ...
    action_selector = s2_mlp.ActionSelector(initial_eps =initial_epsilon, final_eps =final_epsilon, eps_decay =epsilon_decay, random_exp =random_exploration_interval,
    ...
    for i in range(num_eval_episode):
        ...
        for _ in range(10):
            ...
            pixels = s2_mlp.get_frame_tensor(pixels)
...
def main():
    ...
    replay_buffer_capacity, height, width = s2_mlp.get_frame_tensor(env.reset()).shape

    # Get the number of possible actions in the game
    possible_actions = env.action_space.n

    # Initialize the policy and target networks using the S2MLP class
    policy_network = s2_mlp.S2MLP(device, possible_actions).to(device)
    target_network = s2_mlp.S2MLP(device, possible_actions).to(device)
    ...
    memory = s2_mlp.ReplayMemory(memory_size, [frame_stack_size, height, width], device)

    # Initialize the action selector
    action_selector = s2_mlp.ActionSelector(initial_epsilon, final_epsilon, epsilon_decay, random_exploration_interval, policy_network, possible_actions, device,for_evaluation=False)
    ...
    for step in training_progress_bar:
        ...
        training_flag = step > max(random_exploration_interval-previous_experience,0)
