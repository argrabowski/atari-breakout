...
def evaluate(policy_net, device, environment, n_actions, num_episode):
    ...
    action_selector = s2_mlp.ActionSelector(initial_eps=0.015, policy_net=policy_net, n_actions=n_actions, dev=device, for_evaluation=True)
...
def main():
    ...
    policy_net = s2_mlp.S2MLP(possible_actions, device).to(device)
