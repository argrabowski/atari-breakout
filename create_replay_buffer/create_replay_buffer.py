"""
Script Name: create_replay_buffer.py
...
buffer_size = 500000 # Number of samples you want
...
episodes = buffer_size*2                    # Total number of episodes to train for
memory_size = buffer_size                   # Maximum size of the replay buffer
policy_network_update = 5000                # Frequency (in steps) at which to update the policy network
...
random_exploration_interval = buffer_size*2 # Number of steps during which to perform random exploration before using the policy network
...
def main():
    ...
    for step in training_progress_bar:
        ...
        if step > buffer_size:  # Check if the memory has more than 50 elements
            memory.save_replay_memory("../replay_buffer/replay_memory_500k.pt")
            break
